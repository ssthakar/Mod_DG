...
int main()
{
  ...
  std::cout<<"this the number of internal faces"<<mesh1.bface(6,4)<<std::endl;
  ...
  printMatrix(mesh1.fvRkstor,"Rk.out");
  //DG::fv_rhsboun_bface(mesh1);
  //DG::fv_rhsboun_iface(mesh1);
  //printMatrix(mesh1.fvrhsel,"rel.out");
  std::cout<<"boun"<<mesh1.ntype<<std::endl;
  std::cout<<mesh1.esuel(0,1)<<" test "<<std::endl;
  std::cout<<mesh1.bface(7,4)<<" tst1"<<std::endl;
  ...
  {
    file<<mesh1.Ltimestep[i]<<std::endl;
...
//43      0.14968342E+01      0.99996180E-01
  ...
  std::vector<std::vector<double>> soln = reader::readv("unkel2.out");
